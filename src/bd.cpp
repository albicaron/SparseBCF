...
#else
bool bd(tree& x, xinfo& xi, dinfo& di, pinfo& pi, std::vector<size_t>& nv_s, std::vector<double>& pv, RNG& gen)
#endif
{
...
bool bdhet(tree& x, xinfo& xi, dinfo& di, double* phi, pinfo& pi, std::vector<size_t>& nv_s, std::vector<double>& pv, RNG& gen)
{
   tree::npv goodbots;  //nodes we could birth at (split on)
   double PBx = getpb(x,xi,pi,goodbots); //prob of a birth at x
