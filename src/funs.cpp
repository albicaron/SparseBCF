...
/* 
	DIRICHLET SPLITTING PROBABILITY UPDATE 
*/
...
void draw_s(std::vector<size_t>& nv, std::vector<double>& lpv, double& theta, std::vector<double> col_ind, RNG& gen){
  size_t p=nv.size();
// Now draw s, the vector of splitting probabilities
  std::vector<double> _theta(p);  
  for(size_t j=0;j<p;j++) _theta[j]=(col_ind[j]*theta)/(double)p+(double)nv[j];
...
void draw_theta0(bool const_theta, double& theta, std::vector<double>& lpv,
		 double a, double b, double rho, RNG& gen){
